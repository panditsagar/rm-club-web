...
const AboutSection2 = () => {
  ...
  // REMOVED useSpring to prevent conflict with existing SmoothScroll (Lenis).
  // Direct mapping ensures the dot tracks perfectly with the smooth page scroll.
  const smoothProgress = scrollYProgress;

  const lineHeight = useTransform(smoothProgress, [0, 1], ["0%", "100%"]);

  return (
    <div ref={containerRef} className="relative bg-[#080618] pb-20 pt-20 px-6 z-20">
      {/* Top Gradient Blend - Extends UP overlapping previous section */}
...
const TimelineItem = ({ item, index, progress, total }) => {
  ...
  // We want the dot to appear just AFTER the main line passes this point.
  // Using (index + 0.5) / total centers the threshold in the item's "sector".
  const threshold = (index + 0.5) / total;

  // WIDER RANGES for smoother reveal
  // Start revealing much earlier and finish just after the dot passes
  ...
  // Static dot logic - appears right at the threshold
  // Made slightly softer transition
  ...
  return (
    ...
    >
      {/* Static Dot (Milestone) - Controlled by global progress */}
      <motion.div
        style={{ scale: dotScale, opacity: dotOpacity }}
        className="absolute left-1/2 transform -translate-x-1/2 w-5 h-5   z-10 hidden md:block  "
      >
        <img src="/dot.avif" alt="" />
      </motion.div>
      ...
      >
        <div className="relative aspect-video rounded-md overflow-hidden shadow-2xl group">
          <Image
            src={item.image}
      ...
      >
        <h2
          className="font-switzer text-5xl md:text-7xl lg:text-[1.6rem] font-normal tracking-tight leading-none capitalize
 text-[#2D68FF] max-w-5xl"
        >
          {item.year}
        </h2>
        <h2
          className="font-switzer text-5xl md:text-7xl lg:text-[2.8rem] font-normal tracking-tight leading-none capitalize
